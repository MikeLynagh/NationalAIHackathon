...
from data_parser import MPRNDataParser, parse_mprn_file, validate_mprn_data, clean_and_resample
...
def main():
    ...
    page = st.sidebar.selectbox(
        "Navigation",
        ["📊 Data Upload & Analysis", "💰 Usage Patterns", "🔍 Appliance Detection", "💡 Recommendations"]
    )
...
def show_data_upload_page():
    ...
    uploaded_file = st.file_uploader(
        "Choose a CSV file",
        type=['csv'],
        help="Upload your MPRN smart meter data file"
    )
        ...
        try:
            ...
            if os.path.exists(sample_path):
                ...
                if df is not None and not df.empty:
                    ...
                    st.session_state['parsed_data'] = df
                    st.session_state['uploaded_file'] = "sample_mprn.csv"
                    ...
                    try:
                        ...
                        st.info("This is normal for sample data - the parser will handle it automatically.")
                else:
...
def show_basic_statistics(df):
    ...
    if 'timestamp' in df.columns:
        ...
        st.info(f"**Start:** {date_range['min'].strftime('%Y-%m-%d %H:%M')} | **End:** {date_range['max'].strftime('%Y-%m-%d %H:%M')}")
...
def show_usage_patterns_page():
    ...
    if 'timestamp' in df.columns and 'import_kw' in df.columns:
        ...
        df_hourly = df_analysis.set_index('timestamp').resample('H')['import_kw'].mean()
        ...
        weekly_pattern = df_analysis.groupby(['day_of_week', 'hour'])['import_kw'].mean().unstack()
